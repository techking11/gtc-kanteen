...
<section class="layout_padding pt-3">
    <div class="container">
      <a href="me.php" class="btn btn-info">&leftarrow; Back</a>
      <?php
      $stmt = $database->prepare("SELECT * FROM tblmekanteen WHERE id =".$_GET['id']);
      $stmt->execute();
      $result = $stmt->fetch();
      ...
      ?>
        <div class="row mt-4">
          <div class="col-md-12 heading_container"><h2><?php echo $result['menu_name']; ?> - ME KanTeen</h2></div>
          <div class="col-md-6">
              <div class="img-box mt-3">
                ...
                if (isset($_POST['order'])) {
                  $add = $count + 1;
                  $sql = "UPDATE tblmekanteen SET menu_order='$add' WHERE id =".$_GET['id'];
                ...
                <h5 class="d-flex justify-content-between align-items-center mt-3">
                  ...
                  $data = query('SELECT menu_rating FROM tblmekanteen WHERE id ='.$result['id']);
                  ?>
                  <span class="d-flex">
...
<script>
    function countLike() {
        fetch("http://localhost/GTC%20KanTeen/meapi.php?id=<?php echo $result['id']; ?>")
        .then(function(res) {
            return res.json();
        })
        .then(function(data) {
