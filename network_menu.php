...
<section class="layout_padding pt-3">
    <div class="container">
      <a href="network.php" class="btn btn-info">&leftarrow; Back</a>
      <?php
      $stmt = $database->prepare("SELECT * FROM tblnetworkkanteen WHERE id =".$_GET['id']);
      $stmt->execute();
      $result = $stmt->fetch();
      ...
      ?>
        <div class="row mt-4">
          <div class="col-md-12 heading_container"><h2><?php echo $result['menu_name']; ?> - Network KanTeen</h2></div>
          <div class="col-md-6">
              <div class="img-box mt-3">
                <img src="./Admin/images/network/<?php echo $result['menu_img']; ?>" alt="" class="img-fluid rounded" width="100%">
                <?php
                if (isset($_POST['order'])) {
                  $add = $count + 1;
                  $sql = "UPDATE tblnetworkkanteen SET menu_order='$add' WHERE id =".$_GET['id'];
                  $stmt = $database->prepare($sql);
                  $stmt->execute();

                  if ($_SESSION['username']) {
                    $user = $_POST['name'];
                    $kanteen = 'Network KanTeen';
                ...
                <h5 class="d-flex justify-content-between align-items-center mt-3">
                  ...
                  $data = query('SELECT menu_rating FROM tblnetworkkanteen WHERE id ='.$result['id']);
                  ?>
                  <span class="d-flex">
...
<script>
    function countLike() {
        fetch("http://localhost/GTC%20KanTeen/networkapi.php?id=<?php echo $result['id']; ?>")
        .then(function(res) {
            return res.json();
        })
        .then(function(data) {
