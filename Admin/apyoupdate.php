...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="text-info">APyo KanTeen Update Menu</h1>
        </div>
        <div class="col-sm-6">
          ...
          <div class="card">
            ...
            if ($_POST) {
              ...
              if ($_FILES) {
                ...
                $sql = "UPDATE tblapyokanteen SET menu_name='$menu_name', menu_img='$menu_image', menu_content='$menu_content', menu_price='$menu_price' WHERE id ='$id'";
                $stmt = $database->prepare($sql);
                $stmt->execute();
              }else {
                $sql = "UPDATE tblapyokanteen SET menu_name='$menu_name', menu_content='$menu_content', menu_price='$menu_price' WHERE id ='$id'";
                $stmt = $database->prepare($sql);
                $stmt->execute();
              }
              echo "<script>window.location.href='apyokanteen.php'</script>";
            }
            $sql = "SELECT * FROM tblapyokanteen WHERE id =".$_GET['id'];
            $stmt = $database->prepare($sql);
            $stmt->execute();
            $row = $stmt->fetchAll();
            ?>
