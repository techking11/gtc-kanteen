...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="text-info">Network KanTeen Update Menu</h1>
        </div>
        <div class="col-sm-6">
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <a href="index.php" class="btn btn-secondary">&leftarrow; Back</a>
            </div>
            ...
            if ($_POST) {
              ...
              if ($_FILES) {
                $files = 'images/network/'.($_FILES['menu_image']['name']);
                $menu_image = $_FILES['menu_image']['name'];
                move_uploaded_file($_FILES['menu_image']['tmp_name'], $files);

                $sql = "UPDATE tblnetworkkanteen SET menu_name='$menu_name', menu_img='$menu_image', menu_content='$menu_content', menu_price='$menu_price' WHERE id ='$id'";
                $stmt = $database->prepare($sql);
                $stmt->execute();
              }else {
                $sql = "UPDATE tblnetworkkanteen SET menu_name='$menu_name', menu_content='$menu_content', menu_price='$menu_price' WHERE id ='$id'";
                $stmt = $database->prepare($sql);
                $stmt->execute();
              }
              echo "<script>window.location.href='index.php'</script>";
            }
            $sql = "SELECT * FROM tblnetworkkanteen WHERE id =".$_GET['id'];
            $stmt = $database->prepare($sql);
            $stmt->execute();
            $row = $stmt->fetchAll();
            ?>
