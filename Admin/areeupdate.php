...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="text-info">ARee KanTeen Update Menu</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">Home</a></li>
            <li class="breadcrumb-item active">ARee KanTeen</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <a href="areekanteen.php" class="btn btn-secondary">&leftarrow; Back</a>
            </div>
            ...
            if ($_POST) {
              ...
              if ($_FILES) {
                ...
                $sql = "UPDATE tblareekanteen SET menu_name='$menu_name', menu_img='$menu_image', menu_content='$menu_content', menu_price='$menu_price' WHERE id ='$id'";
                $stmt = $database->prepare($sql);
                $stmt->execute();
              }else {
                $sql = "UPDATE tblareekanteen SET menu_name='$menu_name', menu_content='$menu_content', menu_price='$menu_price' WHERE id ='$id'";
                $stmt = $database->prepare($sql);
                $stmt->execute();
              }
              echo "<script>window.location.href='areekanteen.php'</script>";
            }
            $sql = "SELECT * FROM tblareekanteen WHERE id =".$_GET['id'];
            $stmt = $database->prepare($sql);
            $stmt->execute();
            $row = $stmt->fetchAll();
            ?>
