...
<section class="layout_padding pt-3">
    <div class="container">
      <a href="drawing.php" class="btn btn-info">&leftarrow; Back</a>
      <?php
      $stmt = $database->prepare("SELECT * FROM tbldrawingkanteen WHERE id =".$_GET['id']);
      $stmt->execute();
      $result = $stmt->fetch();
      ...
      ?>
        <div class="row mt-4">
          <div class="col-md-12 heading_container"><h2><?php echo $result['menu_name']; ?> - Drawing KanTeen</h2></div>
          <div class="col-md-6">
              <div class="img-box mt-3">
                ...
                if (isset($_POST['order'])) {
                  $add = $count + 1;
                  $sql = "UPDATE tbldrawingkanteen SET menu_order='$add' WHERE id =".$_GET['id'];
                  $stmt = $database->prepare($sql);
                  $stmt->execute();

                  if ($_SESSION['username']) {
                    $user = $_POST['name'];
                    $kanteen = 'Drawing KanTeen';
                ...
                <h5 class="d-flex justify-content-between align-items-center mt-3">
                  ...
                  $data = query('SELECT menu_rating FROM tbldrawingkanteen WHERE id ='.$result['id']);
                  ?>
                  <span class="d-flex">
...
<script>
    function countLike() {
        fetch("http://localhost/GTC%20KanTeen/drawingapi.php?id=<?php echo $result['id']; ?>")
        .then(function(res) {
            return res.json();
        })
        .then(function(data) {
